...
class ListQuotesComponent extends Component
{
    ...
    public function render()
    {
        ...
        $quotes = auth()->user()->quotes()
            ...
            ->leftJoin('quote_information', 'quote_updates.quote_information_id', 'quote_information.id')
            ->where("quote_information.company", "LIKE", $keyWord)
            ->orWhere("quote_information.name", "LIKE", $keyWord)
            ->orWhere("quote_information.oportunity", "LIKE", $keyWord)
            ->select('quotes.*')
            ->groupBy('quotes.id')

        ...
        <div class="flex items-center gap-3 flex-col md:flex-row w-full md:w-auto">
            <div class="flex w-full md:w-auto ">
                ...
                <button data-collapse-toggle="navbar-multi-level" type="button"
                    class="{{ count(auth()->user()->info) > 1 ? 'hidden' : 'inline-flex' }}  items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:border-gray-100 border border-transparent focus:outline-none focus:ring-2 focus:ring-gray-200"
            ...
            <div
                class="md:flex items-center w-full md:w-auto justify-between {{ count(auth()->user()->info) > 1 ? 'flex' : 'hidden' }}">
                {{-- Company Info --}}
                @if (count(auth()->user()->info) > 1)
                    <div class="">
                        <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar"
                            class="flex items-center justify-between w-full py-2 pl-3 pr-4  text-white hover:bg-transparent md:border-0 hover:text-secondary-500 md:p-0 md:w-auto">{{ auth()->user()->companySession->name }}
                ...
                <button data-collapse-toggle="navbar-multi-level" type="button"
                    class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:border-gray-100 border border-transparent focus:outline-none focus:ring-2 focus:ring-gray-200"
                    aria-controls="navbar-multi-level" aria-expanded="false">
                    <span class="sr-only">Open main menu</span>
                    <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 17 14">
            ...
            <ul
                ...
                <li>
                    <a href="{{ route('catalogo') }}"
                        class="py-2 pl-3 pr-4  text-black md:text-white rounded hover:text-secondary-500 md:border-0  md:p-0 flex items-center"
                        aria-current="page">
                ...
                <li>
                    <button id="navMenuLink" data-dropdown-toggle="navMenu"
                        class="py-2 pl-3 pr-4  text-black md:text-white rounded hover:text-secondary-500 md:border-0  md:p-0 flex items-center gap-3 md:gap-1">{{ auth()->user()->name }}
                        <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                            fill="none" viewBox="0 0 10 6">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="m1 1 4 4 4-4" />

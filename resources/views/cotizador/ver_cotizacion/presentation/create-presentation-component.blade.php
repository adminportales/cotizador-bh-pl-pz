<div class="flex h-full gap-3">
    <div class="w-1/2">
        ...
        <div class="flex justify-between mt-5">
            ...
            <button class="bg-green-200 p-3 rounded-md hover:bg-green-300" wire:loading.class='cursor-not-allowed' wire:loading.attr="disabled" onclick="savePPT()">Guardar
                Presentacion</button>
            <button class="bg-green-200 p-3 rounded-md hover:bg-green-300" wire:loading.class='cursor-not-allowed' wire:loading.attr="disabled" onclick="previewPW()">Descargar
                Presentacion</button>
        </div>
    </div>
    <div class="w-1/2 ">
    ...
    <div class="w-1/3 ">
        {{-- Spinnner loading --}}
        <div wire:loading.flex wire:target="previewPresentationPW" class="justify-content-center">
            <div role="status">
                <svg aria-hidden="true"
                    class="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                    viewBox="0 0 100 101" fill="none"
        ...
        @if ($urlPWPreview)
            <iframe src="{{ $urlPWPreview }}" style="width:100%; height:700px" frameborder="0"></iframe>
        @endif
    </div>
    <script>
        function preview() {
        ...
        function previewPW() {
            @this.previewPresentationPW()
        }

        function cerrarPreview() {
            @this.urlPWPreview = null;
        }
